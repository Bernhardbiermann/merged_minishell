...
/*   Updated: 2024/12/19 17:46:29 by aroux            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_redirections(t_cmd *cmd)
{
	...
	redir = cmd->redir;
	i = 0;
	while (redir[i]) // 1912A: seems that Nick handled multiple redir by just having a linked list of redirection and using here a while loop
	{
		open_dup_close(&redir[i]);
		i++;
	}
}
...
void	open_dup_close(t_redirect redir)
{
	...
	else if (redir->type == T_OUTPUT || redir->type == T_APPEND)
	{
		if (redir->type == T_OUTPUT)
			fd = open(redir->trunc, O_WRONLY | O_CREAT | O_TRUNC, 0644);
		else if (redir->type == T_APPEND)
			fd = open(redir->append, O_WRONLY | O_CREAT | O_APPEND, 0644);
		if (fd < 0)
...
void redir_heredoc(t_redirect redir)
{
	...
	write(fd[1], heredoc_content, fr_strlen(heredoc_content)); // 1912A: to adapt to what is effectively our heredoc form and content
	close(fd[1]);
